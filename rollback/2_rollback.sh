...
docker run -t --rm --network netmongo mongo:4.0.2 mongo --host replicaTest/mongo1:27017,mongo2:27017 --quiet --eval 'db.hello.insert({"_id": 1, "msg": "This document will be written on the primary and replicated on the secondary."})' | grep -v NETWORK
...
docker run -t --rm --network netmongo mongo:4.0.2 mongo --host replicaTest/mongo1:27017,mongo2:27017 --quiet --eval 'db.hello.insert({"_id": 2, "msg": "This document will be written ONLY on the primary. It will be rollbacked."})' | grep -v NETWORK

# Show current content of the collection
echo -e "\nContent of the collection:"
docker run -t --rm --network netmongo mongo:4.0.2 mongo --host replicaTest/mongo1:27017,mongo2:27017 --quiet --eval 'db.hello.find()' | grep -v NETWORK
...
docker run -t --rm --network netmongo mongo:4.0.2 mongo --host replicaTest/mongo1:27017,mongo2:27017 --quiet --eval 'db.hello.insert({"_id": 3, "msg": "This document will be written on the new primary and replicated when the secondary comes back up."})' | grep -v NETWORK
